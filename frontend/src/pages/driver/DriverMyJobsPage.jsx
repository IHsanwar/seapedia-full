        ...
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-1">
              ...
              <Badge
                className={`text-xs ${isCompleted ? 'bg-green-100 text-green-700' : isInProgress ? 'bg-[#006b5f]/10 text-[#006b5f]' : 'bg-[#115cb9]/10 text-[#115cb9]'}`}
              >
          ...
          <div className="flex gap-2 pt-2">
            <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
              <DialogTrigger asChild>
                <Button className="flex-1 bg-[#006b5f] hover:bg-[#005a50] text-white rounded-sm" disabled={completing}>
              ...
              <DialogContent className="sm:max-w-sm bg-white rounded-sm">
                <DialogHeader>
                  <DialogTitle className="text-[#722b00]">Selesaikan Pengiriman</DialogTitle>
                  <DialogDescription>
                    Apakah kamu yakin ingin menyelesaikan pengiriman untuk pesanan <strong>#{order?.order_number}</strong>?
                  </DialogDescription>
                </DialogHeader>
                <DialogFooter>
                  <Button variant="outline" onClick={() => setConfirmOpen(false)} className="border-[#006b5f] text-[#006b5f] rounded-sm">Batal</Button>
                  <Button
                    disabled={completing}
                    onClick={async () => {
                    ...
                    className="bg-[#006b5f] hover:bg-[#005a50] text-white rounded-sm"
                  >
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold flex items-center gap-2 text-[#722b00]">
            <Package className="h-6 w-6 text-[#722b00]" />
            Job Saya
          </h1>
